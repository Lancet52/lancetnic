...
CLASS_CRITERIA = {'CELoss': nn.CrossEntropyLoss,
                  'BCELoss': nn.BCELoss}

# Функции потерь для регрессии
REG_CRITERIA = {'MSELoss': nn.MSELoss,
                'L1Loss': nn.L1Loss,
...
#-------Пайплайн для Классификации текстовых и числовых данных------------------ 
class Classification:
    ...
    def train(self,
        ...
        Args:
            ...
            device_name (str): Устройство на котором происходит обучение. По умолчанию 'cuda'.
        """
        ...
        self.model = self.model_name(self.input_size, hidden_size, num_layers, self.num_classes, dropout)
        criterion = CLASS_CRITERIA.get(self.crit_name, nn.CrossEntropyLoss)()
        ...
        trainer = ClassificationTrainer(model=self.model,
                                        criterion=criterion,
                                        optimizer=optimizer,
                                        device=device,
    ...
    def visualize_metrics(self, metrics):
        ...
        self.mtx.train_val_precision(epoch=metrics['epoch'],
                                     val_precision=metrics['val_precision'],
                                     save_folder_path=self.new_folder_path
                                     )
        
        self.mtx.train_val_recall(epoch=metrics['epoch'],
